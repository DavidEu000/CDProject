...
class Sm_Imageslider_Block_List extends Mage_Catalog_Block_Product_Abstract
{
	...
	public function _getProductMediaMobile(){ // Added on March 05,2018
		$items = $this->_getConfig('product_additem_mobile');
		$items = unserialize($items);
		if(empty($items)) return;	
		return $items;
	}

	public function _getProductsMobile(){ // Added on March 05,2018
		$helper = Mage::helper('imageslider/data');
		$items =  $this->_getProductMediaMobile();
		$list = array();
		$i = 0;
		if(!empty($items)) {
