	...
	public function _getList()
	{
		...
		switch ($type_filter) {
			...
			case 'fieldproducts':
				...
				$filters = array('lastest_product', 'best_sales', 'most_viewed');// Commented on Jan 12
				$products = array();
				...
				foreach ($filters as $filter) {
					...
					$products[] = $product; // Commented on Jan 12
					//array_unshift($products, $product); // Commented on Jan 12
				}
			...
			if ($products != null) {
				...
				if (!empty($_products)) {
					...
					foreach ($_products as $_product) {
						...
						$_product->status = $_product->getStatus();
